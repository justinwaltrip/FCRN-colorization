...
def main():
    ...
    # start new wandb run
    if wandb_enabled:
        wandb.init(project="fcrn-colorization", notes="transfer", resume=bool(args.resume))
    ...
    if args.resume:
        ...
        checkpoint = torch.load("result/upproj/color/model_best.pth.tar")
        model = checkpoint["model"].module

        # delte checkpoint to save memory
        del checkpoint
        torch.cuda.empty_cache()
        ...
        model.conv3 = nn.Conv2d(num_channels // 32, out_channels, kernel_size=3, stride=1, padding=1, bias=False)
        model.bilinear = nn.Upsample(size=model.output_size, mode=upsample_mode, align_corners=True if upsample_mode == "bilinear" else None)

        # weight init
        model.conv3.apply(FCRN.weights_init)
